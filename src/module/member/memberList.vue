<template>
  <div>
    ...
    <div class="memberList_content">
      ...
      <div class="pages" v-if="memberList.length!==0">
        <el-pagination
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      pageChange: function () {
        var that = this
        that.memberParams.page = $('.pages .el-pagination .el-pager .active').html()
      ...
      pageSkip: function () {
        var that = this
        that.memberParams.page = that.pageNumber
        rq.allMember(that.memberParams).then(function (data) {
          if (data.result === '成功') {
            console.log(data)
            that.memberList = data.data.data
...
<!--引入样式-->
<style lang="scss" scoped>@import "memberList";</style>
